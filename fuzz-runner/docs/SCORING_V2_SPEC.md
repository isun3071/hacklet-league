...
Range bounds [pinned] from CVSS/VRT; rung points [calib].
...
| Disclosed secret | secrets 001/002 | 7.5–9.8 | P1 | 70–98 | 70 | validated_live→92, high_privilege→98 |

### Tier 2 — high, evidence critical (wide ranges; the flat 40 lived here)

| Class | Probe IDs | CVSS | VRT | Range | Default | Escalators |
|---|---|---|---|---|---|---|
| SSTI | ssti 001 | 4→9.8 | **P4→P1** | 40–98 | 40 | execution_confirmed→98 |
| Unrestricted upload | upload 001/002 | 3→9.8 | **P5→P1** | 15–98 | 15 | execution_confirmed→98 |
| IDOR / BAC | idor 001..005 | 6.5–8.1 | P1 | 30–85 | 30 | cross_user_read→55, sensitive_fields→68, bulk_read→78, cross_user_write→85 |
| Backend RLS exposure | backend 001/002/003 | 7.5–9 | P1 | 40–90 | 40 | sensitive_fields→68, write_confirmed→80, bulk_read/high_privilege→90 |
...
| Stored XSS | xss 002 | 8.0 | P2 | 55–85 | 55 | execution_confirmed→70, cross_user_read→85 |

### Tier 3 — medium (VRT P2–P3 / CVSS 5–6.9)

| Class | Probe IDs | CVSS | VRT | Range | Default | Escalators |
|---|---|---|---|---|---|---|
| Reflected / DOM XSS | xss 001, domxss 001 | 6.1 | P3 | 40–61 | 40 | execution_confirmed→61 |
...
| Missing rate limit | ratelimit 001 | 7.5 | P4 | 15 (→35 no_lockout_confirmed) |
...
| Verbose error / banner | debug 001, exposure 005 | ~5 | P5 | 8 |
| Sourcemap disclosure | exposure 006 | ~4 | P4 | 10 |
| Session cookie flags | session 001..004 | 4–6.5 | P5 ish | httponly 15 / samesite 12 / secure 12 [calib, repriced down from 20/15/15] |

\* Not in the Bugcrowd VRT (CORS, path traversal, host header, splitting, filter inj). These are
**CVSS only anchored** — no web app cross check available, so hold them conservative and flag for
manual review.
...
2. Write the `severity` blocks into the ~50 security YAMLs (≈20 classes).
