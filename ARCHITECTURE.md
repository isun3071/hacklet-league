...
4. Django validates: user is player in this round, round is in build phase, budget not exhausted
...
1. T+24:00 arrives; Django signals all active player rounds via WebSocket
2. Django updates round status to evaluation
3. Workstation receives freeze signal and triggers automatic git push to league-managed repo
4. League's fuzz infrastructure receives push notification, pulls the submitted code
5. League deploys submission to ephemeral container in central infrastructure
6. Central fuzz runner executes both public AND hidden test pools against deployed submission
7. Results written to FuzzResult records — these are the authoritative scored results
8. Submission status updated based on deployment success (completed/dnf/limited)
9. Results visible to judges in their portal for clickaround context

The exact code transfer mechanism (git push vs snapshot upload) is implementation choice that doesn't affect the rest of the architecture. Git push is preferred for developer familiarity. The chapter's firewall must allow workstation outbound git push to league infrastructure.
...
build (T+0:00)
   ↓
evaluation (T+24:00) — concurrent: fuzz runner + judges + players prep pitches
   ↓
pitching (T+36:00)
   ↓
deliberation (T+pitch_end)
   ↓
awards (T+deliberation_end)
   ↓
completed
```
