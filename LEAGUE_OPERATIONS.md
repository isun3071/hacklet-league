...
- **Tier C** maximizes freedom: BYOD substrate, any AI tooling, no enforced budgets, no controlled infrastructure, evaluation written and LLM-judged. Credentialing claims are correspondingly bounded. This is the Minimum Viable Round (MVR) floor: the smallest configuration that genuinely delivers HackLet while honouring player workflow authenticity.

- **Tier B** adds the human judges. Live pitch and cross-examination in front of the four permanent roles, on BYOD substrate. It buys the format's most distinctive credentialing dimension — defence under informed adversarial questioning — and it buys it with *people rather than hardware*, needing no infrastructure a Tier C chapter lacks.

- **Tier A** adds enforced substrate parity: controlled workstations, a firewall that makes the league proxy the only reachable model, structural anti-cheating, comprehensive audit. Credentialing claims are correspondingly strong — global ranking contribution, market-meaningful credentials, employer-interpretable signal.
...
**Tier B (Live Judging)**: the tier where human judges enter. Live pitch and cross-examination before the four permanent roles, on BYOD substrate, 8-12 players per panel, an afternoon rather than a full production. No firewall, no RMM, no broadcast, no on-site inspector. It credentials demonstrated live defence of work built under self-selected substrate — real, and explicitly not a parity claim. Wins contribute to the **Open** global board (the Tier B board; format_spec §7.2). The step up from Tier C is a four-role judge corps, not infrastructure, which makes it the realistic second rung. *(Redefined 2026-07-31. Tier B previously meant "league-hosted substrate with honour-system budgets," which was retired: without a firewall the league cannot claim exclusivity, and budgets were always enforceable server-side, so that definition bought no integrity for real cost. The profile described here was formerly Tier C Extended.)* *See TIER_B_OPERATIONS.md for the full operational template.*

**Tier C (Training Tier / MVR)**: HackLet's training tier and Minimum Viable Round floor. BYOD substrate, no enforced token budgets, full fuzz catalog evaluation, PITCH.md as the written communication artifact, **LLM-judged throughout**. Operates two profiles: the **60-min MVR** (any cohort 8-100+) and **multi-round MVR-days**. The truest expression of "we don't legislate AI usage" — players use whatever tooling fits their workflow. Credentials are chapter-local plus the **Developmental** global board (the Tier C board; format_spec §7.2). The MVR is also the league's R&D infrastructure: every event generates operational data that informs catalog evolution, sysprompt calibration, and format clock validation. A chapter wanting live human judging runs Tier B. *See TIER_C_OPERATIONS.md for the full operational template.*
...
**Function 1 (primary) — League cost control**: wherever the league hosts the substrate — always at Tier A, optionally at any tier — every player prompt costs the league real money. The budget is a **ceiling that stops a runaway loop** from burning a chapter's monthly budget in a single round. This is operational risk management protecting the league's financial sustainability, and it is the function the cap is actually sized for.
...
Function 2 *requires* Function 1 to be enforced — you can't report a number you can't measure. But Function 1 doesn't require Tier A; it requires *the league hosting the AI*, which is an opt-in available at any tier. Wherever the league hosts, the proxy meters server-side and the budget is real — that never depended on a firewall.

What the firewall adds is **exclusivity**, and that is a different property. Without it, a metered budget tells you what a player spent *through the league*, not what they spent in total. So Most Efficient, which needs total usage to mean anything, is a **Tier A award only** — at any other tier a player can simply use their own subscription alongside. On BYOD with no league hosting, neither function applies at all, which is why budgets drop rather than becoming honour-system theatre.
...
- Costs are low at current model pricing ($0.09/M input, $0.18/M output as of 2026-07-31), so a 40-player round costs single-digit to low-tens of dollars even with every player at the ceiling

Cost controls are enforced through:

- Per-player token budgets (**10,000,000** per round, one pool shared across build and pitch preparation, server-enforced) — an **ASSUMED** figure sized to survive agentic context re-sending rather than a calibrated one, and *not currently enforced anywhere*: no proxy exists (format_spec.md §5.5)
