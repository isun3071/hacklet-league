...
**The league does not legislate AI-interaction style.** Where the league hosts AI substrate — always at Tier A, and by opt-in elsewhere — players are served *both* a chat-window interface and an in-IDE agent interface from the same league-controlled infrastructure with a *unified token budget* across all interfaces. Players choose whatever combination of chat-style brainstorming and agent-style execution fits their workflow. This matches how real engineering with AI actually happens: fluid switching between modes, with strategic discipline coming from how the player navigates the substrate rather than which mode the format forces them into. Where the league does not host AI substrate (Tier C, BYOD), players use whatever AI tools they choose. The "Relationship" axis that earlier drafts of the format spec used to distinguish Classical from Agentic has been retired in favor of the unified-substrate model, which is more honest to the format's "we don't legislate how you use AI" thesis (§10).
...
*What HackLet adds: rubrics.* RSAC publishes no rubric and produces no per-judge scores — it is a **selection process** that picks one winner on one night. HackLet is a **measurement instrument**: the Communication axis must mean the same thing in a different city next season, which is what makes it composable with the slop axis (§4.3) and what lets it feed a leaderboard at all. So the same Q&A shape carries four role-weighted rubrics behind it. That addition is the whole reason the two are not interchangeable.

*Where HackLet is deliberately stricter.* RSAC does not sequester its finalists and its legitimacy is not questioned; HackLet does (above), because same-archetype submissions are scheduled back-to-back and concentrate the exposure in a way RSAC's format never creates.

*Where HackLet is deliberately tighter.* The pitch is **60 seconds against RSAC's 180**. This is the largest divergence from the precedent and it is load-bearing: an RSAC judge meets the product cold, whereas a HackLet judge has already used the submission during the evaluation window (§3.1, TIER_A §4). The pitch is therefore argument, not exposition, and does not need three minutes to establish what the artifact is.
...
**Deliberation and Voting Phase**: judges enter explicit deliberation. All four sit together — the nontech stakeholder is in the room with the three technical judges — but the stakeholder scores on a **separate rubric**, because translation and trust to a non-verifier is a different measurement from technical defense, and running it through a technical lens loses the thing that role exists to catch. Judges compare what they witnessed during the blind pitch and cross-examination against clickaround observations, re-visit submissions with player framing context, and score on the technical and stakeholder rubrics (§4.1). Once every rubric score is in, the Communication scores **lock**; only then is the Slop Score **released to the panel** (§4.1). Seeing the findings, the tester may file **contests** (§4.2), which is catalog feedback and changes no score.
...
3. Judges deliberate and submit their rubric scores; once every score is in, Communication scores **lock**.
4. Only then is the Slop Score **released to the panel**, and the tester may file contests (§4.2), which change no score.
...
The league pins one AI model as the substrate for all events using it, and every player at an event faces that same model. Players are free to practise against it on their own, which needs no league guarantee — the model is publicly available and they can use it any time. (An earlier draft promised players "access to practise with the announced model in advance." That promise is **cut**: it described nothing the league was providing.)

Current substrate: **`deepseek/deepseek-v4-flash-0731`**, accessed through OpenRouter, with no league-injected system prompt beyond the model's standard production deployment. The model behaves as it would in any other standard deployment. No league-specific tuning, persona, or behavioral modification.

**Pin the dated snapshot, never a rolling alias.** The ID is exact and deliberate. OpenRouter also publishes `deepseek/deepseek-v4-flash` and `deepseek/deepseek-v4-flash-latest`, both of which move: the 0731 revision landed on 2026-07-31 and re-post-trained the model for coding and agent workflows. Anything pinned to a rolling alias would have changed substrate that day with no action, no announcement, and no record.

The reason is narrower than it may look, and it is **not** cross-season comparability. Model revisions across time are normal and expected, the way sports rules change between seasons without voiding prior records — and HackLet is better insulated than most, because Best Overall is a **rank-based** composition (§4.3) and the leaderboard aggregates placement rather than raw scores, so a model shift moves everyone's absolute output without disturbing ranks within an event at all.

What a rolling alias actually breaks is *within-window predictability*: two events a week apart could run different models, land results on the same leaderboard, and leave nothing anywhere recording that they differed. That is a missing-metadata problem, not an eras problem. A dated pin makes the substrate a known, stated fact for every event — which is also what makes era-qualification possible later, if results are ever broken out by period.
...
**BYOD substrate (Tier B and Tier C)**: neither tier requires league-hosted substrate. Players bring their own laptops and use whatever AI tooling they prefer (chat clients, IDE agents, mix and match). Web search and multiple models are allowed, because without a firewall any restriction is theatre. Token budgets do not apply by default, because the league is not paying for the inference. A chapter at either tier may **opt into league-hosted substrate**, in which case the league meters and pays for it and the budget is genuinely enforced — but exclusivity is not, so the parity claim stays out of reach. What separates Tier B from Tier C is not the substrate at all: it is **live human judging** (LEAGUE_OPERATIONS.md §4).
...
**Why the number is this large, and why it is ASSUMED.** The earlier figure was 100,000, and it was retired for a structural reason rather than an empirical one: **agentic clients re-send their working context on every step**, so cumulative consumption runs orders of magnitude above what is resident in context at any moment. A cap sized against instantaneous context is not a constraint a player works within, it is a number an agent exceeds before finishing its first task. 10M is set well above that failure mode while still being low enough to stop a runaway loop, which is the job the budget actually has.

It is **ASSUMED**, not calibrated. No round has been run on the season model and measured, so nothing here is derived from observation. Re-derive it once real rounds produce real numbers on V4-Flash, and treat the per-timer ladder as blocked on the same thing.
