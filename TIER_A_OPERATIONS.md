...
**Token budgets enforced**: per-player token budget per round (**10M** for Sprint; the per-timer ladder needs re-deriving from this base — format_spec.md §5.5). One pool shared across build and pitch preparation. Enforced server-side at the proxy. Players who exhaust the budget have no AI access remaining for the round, in either phase. *(The former 25k per-prompt cap is not restated here: §5.5 records it as non-functional for agentic use, and whether a per-prompt ceiling survives the rebase to 10M is undecided.)*
