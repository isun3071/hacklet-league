...
**Deliberation + audience voting (18 min)**: all four judges deliberate together, the nontech stakeholder included, but the stakeholder scores on a **separate rubric** from the three technical roles (format_spec.md §3.1). They compare what they witnessed during the blind pitch and cross-examination against clickaround observations, re-visit submissions with player framing context, read README content more carefully, and score across their own rubrics. Once every score is in, Communication scores **lock**; only then is the Slop Score **released to the panel** (format_spec.md §4.1). Seeing the findings for the first time, the tester may file **contests** (format_spec.md §4.2) — catalog feedback that changes no score, which is why the tester's role runs slightly past the round while the other three end when scores lock.
