...
**Chat-window interface**: browser tab from each workstation to hackletleague.com chat window. The interface speaks the league's OpenAI-compatible chat completions endpoint (`/api/v1/chat/completions`). Players access this throughout the build phase and during pitch preparation, on one shared token budget (format_spec.md §5.5). What changes at freeze is not the chat but the artifact: it has already been captured and deployed, so the model can help a player understand what they shipped but cannot alter it.
...
**Token budgets enforced**: per-player token budget per round (**10M** for Sprint; the per-timer ladder needs re-deriving from this base — format_spec.md §5.5). One pool shared across build and pitch preparation. Enforced server-side at the proxy. Players who exhaust the budget have no AI access remaining for the round, in either phase. *(The 25k per-prompt cap is **retired** — at a 10M budget it constrains nothing, and it was already non-functional for agentic clients. A rate-limit throttle protects the proxy instead; see format_spec.md §5.5.)*

The budget is **cost control first and an efficiency signal second** — a ceiling that stops a runaway loop from burning a chapter's month in one round, not a number calibrated to bind on a normal round and force triage (format_spec.md §5.5, LEAGUE_OPERATIONS.md §4). Earlier drafts of this section framed budget exhaustion as *"part of the resource calibration credentialing claim."* That claim is withdrawn until on-substrate data supports it. The **25k per-prompt cap is retired** — it was non-functional for agentic use (a single agentic step already carries more than that in resident context) and pointless against a 10M budget. An ordinary rate-limit throttle takes its place (format_spec.md §5.5). Neither the budget nor the throttle is enforced anywhere today, because no proxy exists.
...
At T+29:00, code freeze takes effect simultaneously. The network cuts for code changes. All build activity ceases — no further coding, no agent edits via the in-IDE interface, no fuzz invocations against the workstation. The submission is captured and deployed at this instant, and the fuzzer grades the deployed copy, so anything the player does to their working directory afterwards reaches nothing that is scored. **The AI substrate does not cut off here** — it stays available through pitch preparation on the same budget (format_spec.md §5.5). Human edits at freeze are the separate rule and are inspector-enforced at Tier A. The submission is what existed at freeze.
...
*Judges evaluate submissions*: judges interact with each submission live in their portals while the fuzz runner completes work. The panel includes four permanent specialized roles (tester, UI/UX/HCI, general engineering, and nontech stakeholder). Because the four roles are permanent and specialized, **every judge scores every player** — the panel does not divide the field. Judges evaluate all 8 submissions across the 18-minute window, and continue examining them during deliberation, which is a second pass; **nothing allocates or enforces per-submission time, and there is no per-submission figure.** What judges have is the running app and clickaround, plus code and README for the three technical roles. **They do not have the Slop Score** — no judge receives it until Communication scores lock (format_spec.md §4.1). Clickaround surfaces what automation can't measure, and the tester's white-box read of the code is a stronger instrument for testing a claim than a probe result would be.
...
- 120 seconds of cross-examination — the four judges work the window as a panel, not on allotted turns, and **blind to the Slop Score** (no judge has the findings, format_spec.md §4.1). The player is scored on responsiveness and concision: answer what was asked, then yield the floor (format_spec.md §3.1)
...
**Deliberation + audience voting (18 min)**: all four judges deliberate together, the nontech stakeholder included, but the stakeholder scores on a **separate rubric** from the three technical roles (format_spec.md §3.1). **The Slop Score is released at the start of this window** — after the blind pitch and cross-examination, with Communication scores locked (format_spec.md §4.1). They compare what they witnessed during pitches against clickaround observations, re-visit submissions with player framing context, read README content more carefully, and score across their own rubrics. Seeing the findings for the first time, the tester may file **contests** (format_spec.md §4.2) — catalog feedback that changes no score, which is why the tester's role runs slightly past the round while the other three end when scores lock.
...
- Judge cognitive load (every judge scores every player, so the panel absorbs the whole field within the 18-min window and again during deliberation)
