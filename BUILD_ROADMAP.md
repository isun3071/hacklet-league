...
## Status & Deviations (as of 2026-08-03)

- **Stages 0–3 shipped.** Stage 1 (Django + DRF + Postgres + Next.js foundation, Google SSO, chapters) landed first; Stage 2 added the `events` app (Event, EventParticipant — invitations, applications, judge corps assignment, event pages, chapter staff management); Stage 3 added the `rounds` and `rankings` apps (Round with the server-authoritative phase clock, Submission via zip upload at code freeze, Score, Ranking) plus the frontend routes through `events/[chapter]/[event]/rounds`, `leaderboard`, and `scoring`. The platform runs at https://hackletleague.com over HTTPS with production settings (DEBUG off, secure cookies, HSTS). It is **public (reachable) but not publicized** — no announcement or marketing; the domain resolves and serves while the platform is built and tested. (This supersedes the earlier "stealth / LAN-only until launch" plan — *public* and *publicized* are treated as separate steps.)
- **Stage 3's scoring stand-in has been replaced by the fuzzer's number (manual bridge).** Stage 3 launched with a judge-entered `engineering_score` as the objective axis because the fuzz runner was Stage 5. That is no longer current: `Submission.slop_score` exists, a manual-bridge grading path (`manage.py import_fuzz_results`) ingests the runner's `--out` grade records into `slop_score` + `FuzzResult` rows, and `rounds/scoring.py` now ranks the objective axis on `slop_score` (ascending) with the role-weighted Communication axis (30/20/20/30). This is the **manual bridge** (operator runs the runner out-of-band, imports the JSONL), not the full networked Stage 5 pipeline. See CHANGELOG.
- **Stage 4 (AI Substrate) is the active stage, and is now unblocked.** No substrate code exists yet — no `ai_proxy` app, no OpenRouter integration, no player portal route. What changed on 2026-08-03 is that the design questions that were blocking it were answered: the gate is **two windows** (build, then pitch prep) with a real cut at build end, **10M tokens** in one pool across both, **403 not 429** with a player-facing body, in-flight streams cut, a rate-limit throttle instead of a per-prompt cap, and Tier A/B only. See format_spec §5.5 and the CHANGELOG. Also shipped that day, ahead of the substrate: the **3-minute upload grace** (`rounds/views.py`, `services.SUBMISSION_GRACE`) that four documents had promised and no code implemented.
- **The docs were reconciled 2026-07-28 through 08-03.** DOC_STATE.md audits every section's status and lists contradictions `C-01`…`C-22`. It is a **snapshot with a correction header**, not a live document — read the header before trusting a row. DECISIONS_OWED.md was deleted on 07-31 once its resolutions had landed in format_spec and its remaining open items were marked `OPEN —` in place; a second source of truth for decisions is what let them drift.
- **Stage 5 is proceeding in parallel, out of order.** The fuzz runner is being built as a standalone project in `fuzz-runner/` alongside Stage 4 rather than after it. This is a deliberate deviation from Rule 1, and the roadmap already anticipates it: the Stage-Tier Readiness Mapping below notes that Tier C launch does not require Stage 4, so Stage 4 and Stage 5 can run concurrently rather than sequentially. The two touch almost no shared code — the runner grades deployed submissions, the substrate serves inference during build — so they interleave cleanly. Rule 1 still holds *within* each track.
...
Tier C and Tier B **do not require** Stage 4 (AI substrate) — both are BYOD, so players bring their own AI. Under the 2026-08-03 tier restructure the ladder grades on judging rigour, not on who hosts the model: Tier C is LLM-judged, Tier B adds live human cross-examination, and only **Tier A requires league-hosted substrate**, because only Tier A's firewall makes it exclusive enough to claim parity. League hosting is otherwise an opt-in at any tier.
...
**Added 2026-08-03 with the tier restructure.** Tier B is no longer "league-hosted substrate" —
