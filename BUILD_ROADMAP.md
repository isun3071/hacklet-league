...
- **Stage 3 shipped with a scoring stand-in.** The engineering axis is a judge-entered `engineering_score` rather than a machine-produced Slop Score, because the fuzz runner that produces the real number is Stage 5. The deduction-only `slop_score` field is born when the runner lands. See CHANGELOG.
