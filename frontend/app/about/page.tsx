...
export default function AboutPage() {
  return (
    <main className="container block">
      ...
      <p className="body">{`I'm Ian Sun, a cybersecurity practitioner and framework developer specializing in human layer security strategy and behavioral risk.`}</p>
      <p className="body">{`I introduced the "Dissonance Test" at SecureWorld Financial Services in 2025, returned to SecureWorld Boston in April 2026 with the bounded vs. unbounded deception lens, hosted sessions at RSAC 2026, and debuted the "Deception Disruption Framework" at Layer 8 in June 2026. With Dr. Faisal Abdullah I co-presented "When Labs Have Stakes" at the NICE Conference, on building instinctive judgment in up-and-coming practitioners where failures have cascading consequences.`}</p>
      <p className="body">{`I finished my BA in CS at Boston University in May 2026 and start a masters degree in CS/Cybersecurity there this fall. I also work IT Support at the BU Help Center.`}</p>
      ...
      <p className="body">{`HackLet compresses building into 24 minutes with AI. When time's up, our fuzzer runs against the submissions. It types junk into your forms to see if the page falls over. It changes the numbers in your links to see if it can read someone else's stuff. It goes looking for passwords you left somewhere public, which happens more than you would think. And it hits the app hard enough to find out whether it stays up. In this manner, no slop survives.`}</p>
      ...
      <p className="body">{`We have three formats. HackLet Vibe (vibecoding from scratch, like a hackathon). HackLet Unslop (fixing AI codebase slop, hence the name), which addresses what most software engineering work actually is: maintaining and fixing existing codebases. And HackLet Underspecified, where you get a deliberately vague client brief and have to decide what it means before you build, then defend that reading. All three go through the same fuzzer.`}</p>
      <p className="body">{`HackLet aims to have three tiers of events, and they differ in who is accountable for the judging. Tier C is the entry-level tier, close to a hackathon: anyone can run one, it fits an hour-long club meeting, and submissions are scored by an AI judge. Tier B adds live human judges. You pitch your work and defend it under cross-examination, still on your own machine and your own AI, with the chapter attesting to how it runs anti-cheating. Tier A adds controlled workstations and a league-supplied AI that everyone shares, so results are directly comparable and feed the global rankings. Tier A is also the only tier we broadcast, though a Tier A event does not have to be televised. The higher the tier, the more rigorous the verification, and the more reliable the achievement.`}</p>
